...
class llm_openai:
    ...
    async def openai_prompt_call_csv(self, convoXmlStr=None, participants=None, minerUid=0):
        ...
        if minerUid > 2:
            prompt1 = f'You are a data tag annotator, with the aim of facilitating personalized conversational AI by offering a comprehensive dataset of indexed and tagged  conversations. Analyze conversation in terms of topic interests of the participants. Analyze the conversation (provided  in structured  XML format) where <p0> has the questions and <p1> has the answers. Return comma-delimited tags. Only return 20 tags without any English commentary.'
        prompt = prompt1 + "\n\n\n"
        if convoXmlStr:
            prompt += convoXmlStr
        else:
